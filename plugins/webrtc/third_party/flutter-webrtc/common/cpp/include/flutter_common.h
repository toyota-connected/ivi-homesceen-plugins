...
inline int toInt(const flutter::EncodableValue& inputVal, int defaultVal) {
...
class MethodCallProxy {
 public:
  ...
  [[nodiscard]] virtual const std::string& method_name() const = 0;

  // The arguments to the method call, or NULL if there are none.
  [[nodiscard]] virtual const EncodableValue* arguments() const = 0;
};
...
class EventChannelProxy {
 public:
  ...
  virtual void Success(const EncodableValue& event, bool cache_event) = 0;
};
